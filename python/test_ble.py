    ...
    async with BleakClient(address) as client:
        while not len(times) or times[-1] < T:
            ...
            if step >= 3:
                ...
                des_torque.append(
                    model(np.array(signal[-3:]).reshape(1, 3) / MAX_SEMG).numpy()[0]
                )
                send_int(client_socket, np.clip(2 * int(semg), 0, MAX_SEMG))
                print(f"Received: {line} Command: {des_torque[-1]}")
